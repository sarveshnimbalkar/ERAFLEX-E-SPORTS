...
export const FloatingChat = () => {
  ...
  const { messages, input, handleInputChange, handleSubmit, isLoading, setInput, sendMessage, error } =
    useChat({
      api: "/api/chat",
      onError: (err) => {
              ...
              <AnimatePresence>
                {error && (
                  <motion.div
                    ...
                    className="flex gap-2.5"
                  >
                    <div className="w-7 h-7 rounded-full bg-red-500/20 border border-red-500/40 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <Bot className="w-3.5 h-3.5 text-red-500" />
                    </div>
                    <div className="flex flex-col gap-1 max-w-[80%]">
                      <div className="bg-red-500/10 border border-red-500/20 px-3.5 py-2.5 rounded-2xl rounded-tl-none text-sm text-red-200 font-indian tracking-wide leading-relaxed">
                        ⚠️ I'm currently unable to connect to the network. Please try again later or contact support.
                      </div>
                    </div>
                  </motion.div>
                )}
